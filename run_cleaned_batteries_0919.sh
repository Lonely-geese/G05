...
TASK=cleaned_batteries_0919
MODE="${1:-train}"
GBS="${GBS:-128}"
case "$GBS" in
  128) PER_GPU=16; EPOCH_STEPS=920 ;;
  *) echo 'GBS must be 128' >&2; exit 2 ;;
esac
case "$MODE" in
...
export EXP_NAME="${EXP_NAME:-right_arm_base_gbs${GBS}_ep10_${MODE}}"
...
p=Path('/mnt/cfs/7rnh3z/kele/0919/mt/cleaned_batteries/training/g05/dataset_stats.json')
r=json.loads(p.with_name('norm_provenance.json').read_text())
assert r['frames']==117689 and r['episodes']==227 and r['stats_downsample_rate']==1
assert hashlib.sha256(p.read_bytes()).hexdigest()==r['stats_sha256']
assert p.with_name('preflight_passed.json').is_file()
assert r['inactive_action_keys']==['left_arm', 'left_gripper']
