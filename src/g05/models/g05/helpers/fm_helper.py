...
class FMHelper:
    ...
    def cal_fm_loss(
        ...
        action_op_mask: torch.BoolTensor = None,
    ) -> torch.Tensor:
        """Velocity MSE loss.
        ...
        Args:
            ...
            action_op_mask: [B, D] or [B, H, D] bool; False means exclude from loss
        Returns:
            scalar loss
        """
        if self.time_convention == "pi_convention":
        ...
        if action_op_mask is not None:
            if action_op_mask.ndim == 2:
                action_op_mask = action_op_mask.unsqueeze(1)
            if action_op_mask.ndim != 3:
                raise ValueError(
                    "action_op_mask must have shape [B, D] or [B, H, D], "
                    f"got {tuple(action_op_mask.shape)}"
                )
            action_weights.masked_fill_(~action_op_mask.bool(), 0.0)
    ...
    def train_step(
        ...
        Args:
            ...
            action_op_mask: [B, D] or [B, H, D] or None; False excludes loss
        ...
        flat_op_mask = (
            action_op_mask.repeat(N, *([1] * (action_op_mask.ndim - 1)))
            if action_op_mask is not None
            else None
        )

        fm_loss = self.cal_fm_loss(
            v_psi, flat_x0, flat_x1, flat_pad_masks, flat_dim_is_pad, flat_op_mask
        )
