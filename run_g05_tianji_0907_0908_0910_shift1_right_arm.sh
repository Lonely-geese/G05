...
PARENT_RUN="$WORKSPACE/g05_runs/r1pro/tianji_0907_0908_shift1_right_arm_8gpu_gbs128_ep5_epoch_ckpt"
# Fine-tune from the final model weights, but start a fresh optimizer/scheduler.
WEIGHT="$PARENT_RUN/last.pt"
...
# Dataset-specific new filename: when absent, finetune.py computes fresh norm stats.
DATA_STATS="$WORKSPACE/g05_runs/stats/tianji_0907_0908_0910_shift1_right_arm_dataset_stats.json"
...
if [[ -e "$DATA_STATS" && "${ALLOW_EXISTING_NORM:-0}" != "1" ]]; then
  echo "Refusing to reuse existing norm stats: $DATA_STATS" >&2
  echo "Set ALLOW_EXISTING_NORM=1 only to reuse stats freshly computed by a verified debug run." >&2
  exit 1
fi
...
export EXP_NAME="${EXP_NAME:-tianji_0907_0908_0910_shift1_right_arm_from_0907_0908_ep5_8gpu_gbs128_ep5}"
