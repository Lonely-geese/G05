...
def main():
    ...
    with initialize_config_dir(str(root / 'configs'), version_base='1.3'):
        cfg = compose(config_name='train', overrides=['task=combined_rj45_0917_0918'])
    ...
    assert len(dataset) == 951339, len(dataset)
    ...
    provenance = {
        'datasets': ['/mnt/cfs/7rnh3z/kele/0917/cleaned_rj45', '/mnt/cfs/7rnh3z/kele/0918/cleaned_rj45'],
        'frames': len(dataset), 'episodes': 2615,
    ...
    sample = dataset[0]
    for index in [0, 535557, 535558, 951338]:
        check_sample=dataset[index]
        check_mask=check_sample['action_op_mask']
        assert check_mask[..., :7].all() and check_mask[..., 9].all()
        assert not check_mask[..., 10:20].any()
