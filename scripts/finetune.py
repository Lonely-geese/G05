...
def finetune(cfg: DictConfig):
    ...
    # --dry-run: short training validation, no checkpoint/tokenizer saves.
    _dry_run = os.environ.get("DRY_RUN", "0") == "1"
    _dry_run_steps = int(os.environ.get("DRY_RUN_STEPS", "1")) if _dry_run else 1
    if _dry_run_steps < 1:
        raise ValueError("DRY_RUN_STEPS must be positive")
    if _dry_run:
        logger.info(
            f"[Dry Run] Training {_dry_run_steps} steps + 1 eval step, only log files will be saved (no checkpoints/tokenizer)"
        )
    ...
    if _dry_run:
        max_steps = _dry_run_steps
        cfg.eval_steps = _dry_run_steps
