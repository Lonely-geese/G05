...
def main():
    ...
    if checkpoint.get('epoch') != 2:
        raise ValueError(f"Expected zero-based epoch index 2, got {checkpoint.get('epoch')}")
    ...
    metadata = {
        ...
        'completed_epochs': 3, 'original_bytes': before.st_size,
    ...
    (staging / 'README.txt').write_text(
        'G0.5 0917+0918 combined_rj45 第 3 个 epoch 推理模型包（22,299 步）。\n'
        ...
        '全部模型权重与第 3 个 epoch checkpoint 逐元素一致，未发现非有限值；CPU 模型/处理器加载通过。\n'
