...
REPO = Path('/mnt/cfs/7rnh3z/kele/GalaxeaVLA')
...
def main():
    ...
    parser.add_argument('--skip-model-setup', action='store_true', help='Package without model initialization when the environment lacks a CUDA driver required by Triton imports')
    ...
    before = source.stat()
    print(f'Loading epoch checkpoint: {source}', flush=True)
    checkpoint = torch.load(source, map_location='cpu', weights_only=False, mmap=True)
    if checkpoint.get('step') != args.expected_step:
        raise ValueError('Checkpoint step does not match expected step')
    if checkpoint.get('epoch') != 7:
        raise ValueError(f"Expected zero-based epoch index 7, got {checkpoint.get('epoch')}")
    ...
    metadata = {
        ...
        'source_epoch': checkpoint['epoch'], 'source_epoch_index_base': 0,
        'completed_epochs': 8, 'original_bytes': before.st_size,
    ...
    if OmegaConf.is_interpolation(raw_cfg.model.model_arch, 'AT_CONFIG'):
        raw_cfg.model.model_arch.AT_CONFIG = OmegaConf.create(OmegaConf.to_container(raw_cfg.model.tokenizer.vq_config, resolve=False))
    ...
    for key in ['ckpt_path', 'datastatics_path', 'model.model_arch.hf_processor_path',
                'model.processor.tokenizer_params.pretrained_model_name_or_path',
                'tokenizer.vq_config.ckpt_dir', 'model.tokenizer.vq_config.ckpt_dir',
                'model.model_arch.AT_CONFIG.ckpt_dir']:
        path = Path(OmegaConf.select(check_cfg, key)).resolve()
        if not path.exists() or not path.is_relative_to(staging):
            raise ValueError(f'Non-portable or missing inference asset: {key}: {path}')
    metadata['inference_asset_paths_verified'] = True
    metadata['cpu_inference_setup_verified'] = False
    metadata['gpu_action_inference_tested'] = False
    if args.skip_model_setup:
        metadata['model_setup_skip_reason'] = 'CUDA unavailable in export environment; model imports require an active Triton driver even for CPU setup.'
        setup_note = '已核验推理资源路径；当前导出环境无法访问 CUDA，未执行模型加载测试。'
        print(metadata['model_setup_skip_reason'], flush=True)
    else:
        ...
        setup_note = 'CPU 模型/处理器加载通过。'
        del policy, processor
        gc.collect()
    (staging / 'export_meta.json').write_text(json.dumps(metadata, indent=2) + '\n')
    (staging / 'README.txt').write_text(
        'G0.5 0917+0918 combined_rj45 第 8 个 epoch 推理模型包（59,464 步）。\n'
        '初始化模型：g05-base；训练数据：combined_rj45_0917_0918；左臂为主动臂。\n'
        f'来源：{source}\n'
        ...
        '全部模型权重与第 8 个 epoch checkpoint 逐元素一致，未发现非有限值。\n'
        f'{setup_note}\n'
    ...
    with archive_partial.open('wb') as out:
        ...
        gzip = subprocess.Popen(['pigz', '-1', '-p', str(args.threads)], stdin=tar.stdout, stdout=out)
        tar.stdout.close()
